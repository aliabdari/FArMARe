...
from DNNs import FCNet, GRUNet, OneDimensionalCNN
import torch
from torch.optim.lr_scheduler import StepLR
from torch.utils.data import DataLoader, Subset
from Data_utils import DescriptionSceneDatasetMem, DescriptionSceneDataset
...
def start_train():
    ...
    model_descriptor = GRUNet(hidden_size=output_feature_size, num_features=768, is_bidirectional=is_bidirectional)
    model_scene = OneDimensionalCNN(in_channels=2048, out_channels=512, kernel_size=5, input_size=2048, feature_size=output_feature_size)
    ...
    ret_data = train_utilities.get_entire_data(clip_ones=False, is_tag_need=False, is_synopsis=False, token_level=is_token_level)
    ...
    train_loader = DataLoader(train_subset, batch_size=batch_size, collate_fn=collate_fn, num_workers=4)
    val_loader = DataLoader(val_subset, batch_size=batch_size, collate_fn=collate_fn, num_workers=4)
    test_loader = DataLoader(test_subset, batch_size=batch_size, collate_fn=collate_fn, num_workers=4)
    ...
    for epoch in tqdm(range(num_epochs)):
        ...
        for i, (data_description, data_scene, list_length) in enumerate(train_loader):
            ...
            output_scene = model_scene(data_scene, list_length)
        ...
        with torch.no_grad():
            for j, (data_description, data_scene, list_length) in enumerate(val_loader):
                data_description = data_description.to(device)
                data_scene = data_scene.to(device)
        ...
        r1, r5, r10, _, _, _, _, _, _, _ = train_utilities.evaluate(output_description=output_description_val,
                                                                    output_scene=output_scene_val,
                                                                    section="val")
        r10_hist.append(r10)
        if r10 > best_r10:
            best_r10 = r10
            train_utilities.save_best_model(model_scene.state_dict(), model_descriptor.state_dict(), 'resnet_one_dim.pt')
    ...
    best_model_state_dict_scene, best_model_state_dict_description = train_utilities.load_best_model('resnet_one_dim.pt')
    ...
    ds1, ds5, ds10, sd1, sd5, sd10, ndgc_10, ndcg, ds_medr, sd_medr = train_utilities.evaluate(
        output_description=output_description_test,
        output_scene=output_scene_test,
        section="test")
    train_utilities.write_models_evaluation_to_file([ds1, ds5, ds10, sd1, sd5, sd10, ndgc_10, ndcg, ds_medr, sd_medr],
                                                    'resnet_one_dimensional' + "_test_evaluate" + ".txt")
    # ds1, ds5, ds10, sd1, sd5, sd10, ndgc_10, ndcg, ds_medr, sd_medr = train_utilities.evaluate_model(
