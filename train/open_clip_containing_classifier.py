...
from Data_utils import DescriptionSceneDatasetMem, DescriptionSceneDataset, DescriptionSceneDatasetMemClassifier
...
def start_train():
    ...
    model_scene = OneDimensionalCNNVClassifier(in_channels=512, out_channels=512, kernel_size=5, input_size=512, feature_size=output_feature_size)
    ...
    is_synopsis = False
    ret_data = train_utilities.get_entire_data(clip_ones=True, token_level=is_token_level, is_tag_need=False, is_synopsis=False)
    ...
    for epoch in tqdm(range(num_epochs)):
        ...
        for i, (data_description, data_scene, data_tag, length) in enumerate(train_loader):
            ...
            # print('conv shape', output_conv.shape)
            input_classifier = create_tensor_classifier(output_conv, length)
            # print('classifier', input_classifier.shape)
            # print('classifier transpose', torch.transpose(input_classifier, 0, 1).shape)
            output_classifier = model_classifier(torch.transpose(input_classifier, 0, 1).to(device))
            # print('output classifier', output_classifier.shape)
            # print('data tag shape', data_tag.shape)
            loss_classifier = criterion_classifier(output_classifier, data_tag)
            loss_classifier = adjust_loss(loss_classifier, data_tag)
            # print('loss ', loss_classifier.shape)
            ...
            loss_contrastive = contrastive_loss(multiplication, targets=ground_truth)
        ...
        if r10 > best_r10:
            best_r10 = r10
            train_utilities.save_best_model(model_scene.state_dict(), model_descriptor.state_dict(), 'open_clip_classifier' + '.pt')
    ...
    best_model_state_dict_scene, best_model_state_dict_description = train_utilities.load_best_model('open_clip_classifier' + '.pt')
    ...
    train_utilities.write_models_evaluation_to_file([ds1, ds5, ds10, sd1, sd5, sd10, ndgc_10, ndcg, ds_medr, sd_medr],
                                                  'open_clip_classifier' + "_test_evaluate" + ".txt")
