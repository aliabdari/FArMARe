...
def start_train():
    ...
    model_scene = FCNet(input_size=512, feature_size=output_feature_size)
    ...
    # Loading Data
    is_token_level = True
    ret_data = train_utilities.get_entire_data(clip_ones=True, token_level=is_token_level, is_synopsis=False, is_tag_need=False)
    ...
    dataset = DescriptionSceneDataset(data_description_, data_scene_, type_model_scene=Constants.model_scene_mean)
    ...
    for epoch in tqdm(range(num_epochs)):
        ...
        r1, r5, r10, _, _, _, _, _, _, _ = train_utilities.evaluate(
            output_description=output_description_val,
            output_scene=output_scene_val,
            section="val")
        # r1, r5, r10, _, _, _, _, _, _, _ = train_utilities.evaluate_model(model_descriptor=model_descriptor,
        ...
        #                                                                   indices=val_indices,
        #                                                                   type_model_desc="gru",
        #                                                                   type_model_scene=Constants.model_scene_mean,
        #                                                                   data_tag=None,
        #                                                                   is_tag_used=False)
        r10_hist.append(r10)
        if r10 > best_r10:
            best_r10 = r10
            train_utilities.save_best_model(model_scene.state_dict(), model_descriptor.state_dict(), 'open_clip' + '.pt')
    ...
    best_model_state_dict_scene, best_model_state_dict_description = train_utilities.load_best_model('open_clip' + '.pt')
    ...
    # Evaluate test set
    with torch.no_grad():
        for j, (data_description, data_scene) in enumerate(test_loader):
            data_description = data_description.to(device)
            data_scene = data_scene.to(device)
            output_descriptor = model_descriptor(data_description)
            output_scene = model_scene(data_scene)
    ...
    train_utilities.write_models_evaluation_to_file([ds1, ds5, ds10, sd1, sd5, sd10, ndgc_10, ndcg, ds_medr, sd_medr],
                                                    'open_clip_mean' + "_test_evaluate" + ".txt")
