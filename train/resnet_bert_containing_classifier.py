...
def start_train():
    ...
    model_scene = OneDimensionalCNNVClassifier(in_channels=2048, out_channels=512, kernel_size=5, input_size=512, feature_size=output_feature_size)
    ...
    is_synopsis = False
    ret_data = train_utilities.get_entire_data(clip_ones=False, token_level=is_token_level, is_tag_need=False, is_synopsis=False)
    ...
    for epoch in tqdm(range(num_epochs)):
        ...
        if r10 > best_r10:
            best_r10 = r10
            train_utilities.save_best_model(model_scene.state_dict(), model_descriptor.state_dict(), 'resnet_classifier' + '.pt')
    ...
    best_model_state_dict_scene, best_model_state_dict_description = train_utilities.load_best_model('resnet_classifier' + '.pt')
    ...
    train_utilities.write_models_evaluation_to_file([ds1, ds5, ds10, sd1, sd5, sd10, ndgc_10, ndcg, ds_medr, sd_medr],
                                                  'resnet_classifier' + "_test_evaluate" + ".txt")
