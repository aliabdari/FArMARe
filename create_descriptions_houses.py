...
def create_descriptions(houses_data):
    ...
    for house_idx, house in tqdm(enumerate(houses_data), total=len(houses_data)):
        ...
        living_rooms_sent = ''
        if no_living_room == 1:
            living_rooms_sent += 'Regarding the ' + get_type_room(living_rooms[0]['type']) + ', it ' + get_verb() + ' '
        ...
        bedrooms_sent = ''
        if no_bedrooms == 1:
            bedrooms_sent += ' Regarding the ' + get_type_room(bedrooms[0]['type']) + ', it ' + get_verb() + ' '
        ...
        desc_features = get_embeddings(entire_desc, model=model, tokenizer=tokenizer, device=device)
        desc_features = desc_features.cpu()
        torch.save(desc_features, './description_features/' + house['json_file'] + '.pt')

        with open('./description_sentences/' + house['json_file'] + '.txt', 'w') as f:
            f.write(entire_desc)
